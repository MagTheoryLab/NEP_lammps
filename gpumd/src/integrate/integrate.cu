...
{
  if (type == kTypeUnset) {
    PRINT_INPUT_ERROR("Please set ensemble before run.\n");
  }
  ...
  switch (type) {
    case 0: // NVE
      ...
      break;
    case kTypeFrozen: // Frozen lattice (no atomic motion)
      ensemble.reset(new Ensemble_Frozen(type));
      CHECK(gpuMemset(
        atom.velocity_per_atom.data(), 0, sizeof(double) * static_cast<size_t>(atom.velocity_per_atom.size())));
      break;
    ...
    case 33: // PIMD
      ...
      break;
    default:
      PRINT_INPUT_ERROR("Illegal integrator!\n");
      break;
  }

  if (!ensemble) {
    PRINT_INPUT_ERROR("Internal error: ensemble is not initialized.\n");
  }
  ...
  ensemble->move_velocity[0] = move_velocity[0];
  ensemble->move_velocity[1] = move_velocity[1];
  ensemble->move_velocity[2] = move_velocity[2];
...
void Integrate::compute1(
  ...
  GPU_Vector<double>& thermo)
{
  if (type == 0 || type == kTypeFrozen || type == 31 || type == 32) {
