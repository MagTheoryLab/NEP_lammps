...
// Sort NL (and associated neighbor vectors) for each atom by neighbor index.
// The vector arrays must use the same [atom + i*N] layout as NL.
static __global__ void gpu_sort_neighbor_list_with_vectors(
  const int N, const int* NN, int* NL, float* x12, float* y12, float* z12)
{
  int bid = blockIdx.x;
  int tid = threadIdx.x;
  int neighbor_number = NN[bid];
