  ...
  const char** param, int num_param, const Box& box, const int number_of_atoms)
{
  ...
  if (strcmp(potential_name, "tersoff_1989") == 0) {
    ...
    is_nep = true;
    check_types(param[1]);
  } else if (strcmp(potential_name, "nep3_spin") == 0 || strcmp(potential_name, "nep4_spin") == 0) {
...
const GPU_Vector<int>* Force::get_spin_freeze_mask() const
{
  return (spin_freeze_mask_.size() > 0) ? &spin_freeze_mask_ : nullptr;
}
...
static __global__ void initialize_mforce(int N, double* g_mforce)
{
  int n1 = blockIdx.x * blockDim.x + threadIdx.x;
  if (n1 < N) {
    g_mforce[n1] = 0.0;
...
static __global__ void gpu_average_mforce(int N, double* g_mforce, double denominator)
{
  int n1 = blockIdx.x * blockDim.x + threadIdx.x;
  if (n1 < N) {
    g_mforce[n1] /= denominator;
    g_mforce[n1 + N] /= denominator;
    g_mforce[n1 + 2 * N] /= denominator;
  }
}

static __global__ void init_mask_ones(int N, int* mask)
{
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < N) {
    mask[i] = 1;
...
static __global__ void apply_spin_zero_mask(int N, const double* spin, int* mask, double eps2)
{
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < N) {
    double sx = spin[i];
    double sy = spin[i + N];
    double sz = spin[i + 2 * N];
...
static __global__ void apply_spin_freeze_mask(int N, const int* mask, double* mforce)
{
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < N && mask[i] == 0) {
    mforce[i] = 0.0;
...
static __global__ void zero_spin_for_frozen(int N, const int* mask, double* spin)
{
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < N && mask[i] == 0) {
    spin[i] = 0.0;
...
void Force::finalize()
{
  ...
  compute_hnemdec_ = -1;
  spin_enabled_ = false;
  mforce = GPU_Vector<double>();
  spin_per_atom_buffer_ = GPU_Vector<double>();
  spin_freeze_mask_ = GPU_Vector<int>();
}
  ...
  GPU_Vector<double>* spin_per_atom)
{
  ...
  const bool use_mforce = spin_enabled_ || (spin_per_atom != nullptr);
  const int grid_atoms = (number_of_atoms - 1) / 128 + 1;
  const bool need_spin_mask = use_mforce && (spin_per_atom != nullptr);
  if (need_spin_mask) {
    if (spin_freeze_mask_.size() < static_cast<size_t>(number_of_atoms)) {
      spin_freeze_mask_.resize(static_cast<size_t>(number_of_atoms));
    }
    init_mask_ones<<<grid_atoms, 128>>>(number_of_atoms, spin_freeze_mask_.data());
    GPU_CHECK_KERNEL
    if (spin_per_atom != nullptr) {
      // if initial spin magnitude is ~0, freeze it
      apply_spin_zero_mask<<<grid_atoms, 128>>>(
        number_of_atoms, spin_per_atom->data(), spin_freeze_mask_.data(), 1.0e-16);
      GPU_CHECK_KERNEL
      zero_spin_for_frozen<<<grid_atoms, 128>>>(
        number_of_atoms, spin_freeze_mask_.data(), spin_per_atom->data());
      GPU_CHECK_KERNEL
    }
  } else {
    spin_freeze_mask_.resize(0);
  }

  if (use_mforce) {
    if (mforce.size() < static_cast<size_t>(3 * number_of_atoms)) {
      mforce.resize(static_cast<size_t>(3 * number_of_atoms));
    }
    initialize_mforce<<<grid_atoms, 128>>>(number_of_atoms, mforce.data());
    GPU_CHECK_KERNEL
  }

  initialize_properties<<<grid_atoms, 128>>>(
    number_of_atoms,
    force_per_atom.data(),
  ...
  auto call_potential = [&](Potential* pot) {
    if (auto* nep_spin_ptr = dynamic_cast<NEP_Spin*>(pot)) {
      GPU_Vector<double>* spin_ptr = spin_per_atom;
      if (spin_ptr == nullptr) {
        if (spin_per_atom_buffer_.size() < static_cast<size_t>(3 * number_of_atoms)) {
          spin_per_atom_buffer_.resize(static_cast<size_t>(3 * number_of_atoms), 0.0);
        }
        spin_ptr = &spin_per_atom_buffer_;
      }
      if (spin_ptr->size() < static_cast<size_t>(3 * number_of_atoms)) {
        PRINT_INPUT_ERROR("Spin vector size is inconsistent with number of atoms.\n");
      }
      GPU_Vector<double>& field_ref = mforce;
      nep_spin_ptr->compute_with_spin(
        box,
        type,
        position_per_atom,
        *spin_ptr,
  ...
  if (multiple_potentials_mode_.compare("observe") == 0) {
    ...
    GPU_CHECK_KERNEL
    if (use_mforce) {
      gpu_average_mforce<<<grid_atoms, 128>>>(number_of_atoms, mforce.data(), (double)potentials.size());
      GPU_CHECK_KERNEL
    }
  } else {
  ...
  if (need_spin_mask && use_mforce && spin_freeze_mask_.size() == static_cast<size_t>(number_of_atoms)) {
    apply_spin_freeze_mask<<<grid_atoms, 128>>>(number_of_atoms, spin_freeze_mask_.data(), mforce.data());
    GPU_CHECK_KERNEL
  }
