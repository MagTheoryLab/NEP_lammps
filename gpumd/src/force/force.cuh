...
public:
  ...
  void compute(
    ...
    GPU_Vector<double>* spin_per_atom = nullptr);
  ...
  void set_spin_enabled(bool enabled) { spin_enabled_ = enabled; }
  ...
  const GPU_Vector<int>* get_spin_freeze_mask() const;
  ...
  GPU_Vector<double> mforce; // per-atom effective spin field H_R = -dE/dS (eV)
...
private:
  ...
  GPU_Vector<double> spin_per_atom_buffer_;
  GPU_Vector<int> spin_freeze_mask_;
