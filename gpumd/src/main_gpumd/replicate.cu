...
void Replicate(const char** param, int num_param, Box& box, Atom& atoms, std::vector<Group>& groups)
{
  ...
  new_atoms.cpu_spin.resize(N * 3);
  const bool has_spin = (atoms.cpu_spin.size() == static_cast<size_t>(n * 3));
  int cur = 0;
  for (int i = 0; i < r[0]; i++) {
    for (int j = 0; j < r[1]; j++) {
      for (int k = 0; k < r[2]; k++) {
        for (int nn = 0; nn < atoms.number_of_atoms; nn++) {
          ...
          for (int d = 0; d < 3; d++) {
            ...
            new_atoms.cpu_spin[cur + d * N] = has_spin ? atoms.cpu_spin[nn + d * n] : 0.0;
          }
          cur++;
        }
      }
  ...
  atoms.cpu_spin.assign(new_atoms.cpu_spin.begin(), new_atoms.cpu_spin.end());
