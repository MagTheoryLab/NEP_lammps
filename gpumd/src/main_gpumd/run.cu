...
void Run::perform_a_run()
{
  ...
  spin_integrate.initialize(atom, number_of_steps);
  mc.initialize();
  ...
  if (integrate.type >= 31 && integrate.type <= 33) { // PIMD
  ...
  for (int step = 0; step < number_of_steps; ++step) {
    ...
    double lattice_T =
      (integrate.ensemble) ? integrate.ensemble->temperature : integrate.temperature2;
    spin_integrate.compute(
      time_step,
      double(step) / number_of_steps,
      atom,
      force.mforce,
      lattice_T,
      force.get_spin_freeze_mask());
...
void Run::parse_one_keyword(std::vector<std::string>& tokens)
{
  ...
  } else if (strcmp(param[0], "ensemble_spin") == 0) {
    spin_integrate.parse_ensemble_spin(param, num_param);
    if (spin_integrate.type != 0) {
      force.set_spin_enabled(true);
    }
...
void Run::parse_velocity(const char** param, int num_param)
{
  ...
  // For NVE-like runs (including cases where ensemble has not been set yet),
  // keep a consistent "lattice temperature" value for downstream features
  // (e.g., temperature-dependent NEP, ensemble_spin with Tmode=lattice).
  if (integrate.type == Integrate::kTypeUnset || integrate.type == 0) {
    integrate.temperature = initial_temperature;
    integrate.temperature1 = initial_temperature;
    integrate.temperature2 = initial_temperature;
  }
}
...
void Run::parse_run(const char** param, int num_param)
{
  ...
  if (integrate.type == Integrate::kTypeUnset) {
    if (spin_integrate.type != 0) {
      integrate.type = Integrate::kTypeFrozen;
      integrate.temperature = 0.0;
      integrate.temperature1 = 0.0;
      integrate.temperature2 = 0.0;
      printf(
        "Note: 'ensemble' is not set but 'ensemble_spin' is set; freeze atomic motion and evolve spins only.\n");
    } else {
      PRINT_INPUT_ERROR("Please set ensemble before run (or set ensemble_spin for spin-only runs).\n");
    }
  }
