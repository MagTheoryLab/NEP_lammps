  ...
  std::vector<Group>& group)
{
  ...
  std::string property_name[num_properties] = {
    "species", "pos", "mass", "charge", "vel", "group", "spins"};
  int property_position[num_properties] = {
    -1, -1, -1, -1, -1, -1, -1}; // species,pos,mass,charge,vel,group,spins
  for (int n = 0; n < tokens.size(); ++n) {
  ...
  int& has_velocity_in_xyz, int& number_of_types, Box& box, std::vector<Group>& group, Atom& atom)
{
  ...
  int property_offset[7] = {0, 0, 0, 0, 0, 0, 0}; // species,pos,mass,vel,group,spins
  int num_columns = 0;
  bool has_mass = true;
  bool has_charge = true;
  bool has_spin = false;
  read_xyz_line_2(
    input, box, has_velocity_in_xyz, has_mass, has_charge, has_spin, num_columns, property_offset, group);
...
void allocate_memory_gpu(std::vector<Group>& group, Atom& atom, GPU_Vector<double>& thermo)
{
  ...
  // Initialize spin if provided on host; otherwise leave empty for spin integrators to allocate.
  if (atom.cpu_spin.size() == static_cast<size_t>(N * 3)) {
    atom.spin.resize(N * 3);
    atom.spin.copy_from_host(atom.cpu_spin.data());
  } else {
    atom.spin.resize(N * 3, 0.0);
  }
  thermo.resize(12);
}
