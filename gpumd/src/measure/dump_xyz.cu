  ...
  Force& force)
{
  ...
  if (quantities.has_spin_) {
    if (atom.spin.size() < static_cast<size_t>(atom.number_of_atoms * 3)) {
      PRINT_INPUT_ERROR("dump_xyz requires spin data but atom.spin is not allocated.\n");
    }
    atom.spin.copy_to_host(cpu_spin_per_atom_.data());
  }
  if (quantities.has_mforce_) {
    auto& mforce = force.mforce;
    if (mforce.size() < static_cast<size_t>(atom.number_of_atoms * 3)) {
      PRINT_INPUT_ERROR("dump_xyz requires magnetic force data but mforce is not allocated.\n");
    }
  ...
  for (int n = 0; n < number_of_atoms_to_dump; n++) {
    ...
    if (quantities.has_spin_) {
      for (int d = 0; d < 3; ++d) {
        fprintf(fid_, " %.8f", static_cast<double>(cpu_spin_per_atom_[m + atom.number_of_atoms * d]));
      }
    }
    if (quantities.has_unwrapped_position_) {
      for (int d = 0; d < 3; ++d) {
    ...
    if (quantities.has_mforce_) {
      for (int d = 0; d < 3; ++d) {
        fprintf(fid_, " %.8f", static_cast<double>(cpu_mforce_[m + atom.number_of_atoms * d]));
      }
    }
    fprintf(fid_, "\n");
  }
