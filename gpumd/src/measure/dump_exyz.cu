...
void Dump_EXYZ::parse(const char** param, int num_param)
{
  ...
  if (num_param >= 7) {
    if (!is_valid_int(param[6], &has_spin_)) {
      PRINT_INPUT_ERROR("has_spin should be an integer.");
    }
    printf("    %s spin data.\n", (has_spin_ ? "with" : "without"));
  }

  if (num_param >= 8) {
    if (!is_valid_int(param[7], &has_mforce_)) {
      PRINT_INPUT_ERROR("has_mforce should be an integer.");
    }
    printf("    %s magnetic force data.\n", (has_mforce_ ? "with" : "without"));
  }
}
  ...
  Force& force)
{
  ...
  if (has_spin_) {
    cpu_spin_per_atom_.resize(static_cast<size_t>(atom.number_of_atoms) * 3);
  }
  if (has_mforce_) {
    cpu_mforce_.resize(static_cast<size_t>(atom.number_of_atoms) * 3);
  }
}
}
  ...
  Force& force)
{
  ...
  if (has_spin_) {
    if (atom.spin.size() < static_cast<size_t>(num_atoms_total * 3)) {
      PRINT_INPUT_ERROR("dump_exyz requires spin data but atom.spin is not allocated.\n");
    }
    atom.spin.copy_to_host(cpu_spin_per_atom_.data());
  }
  if (has_mforce_) {
    auto& mforce = force.mforce;
    if (mforce.size() < static_cast<size_t>(num_atoms_total * 3)) {
      PRINT_INPUT_ERROR("dump_exyz requires magnetic force data but mforce is not allocated.\n");
    }
    mforce.copy_to_host(cpu_mforce_.data());
  }
  ...
  for (int n = 0; n < num_atoms_total; n++) {
    ...
    if (has_spin_) {
      for (int d = 0; d < 3; ++d) {
        fprintf(fid_, " %.8f", static_cast<double>(cpu_spin_per_atom_[n + num_atoms_total * d]));
      }
    }
    if (has_mforce_) {
      for (int d = 0; d < 3; ++d) {
        fprintf(fid_, " %.8f", static_cast<double>(cpu_mforce_[n + num_atoms_total * d]));
      }
    }
    fprintf(fid_, "\n");
  }
