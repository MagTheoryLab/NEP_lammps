  ...
  if (generation == 0) {
    ...
    for (int n = 0; n < population_iter; ++n) {
      ...
      for (int m = 0; m < deviceCount; ++m) {
        ...
        std::vector<float> rmse_mforce_para_array(para.num_types + 1, 0.0f);
        std::vector<float> rmse_mforce_perp_array(para.num_types + 1, 0.0f);
        std::vector<float> rmse_mforce_perp_angle_array(para.num_types + 1, 0.0f);
        ...
        if (para.spin_mode > 0 &&
          ...
          if (para.lambda_m_para > 0.0f)
            rmse_mforce_para_array = train_set[batch_id][m].get_rmse_mforce_para(para, true, m);
          if (para.lambda_m_perp > 0.0f)
            rmse_mforce_perp_array = train_set[batch_id][m].get_rmse_mforce_perp(para, true, m);
          if (para.lambda_m_perp_angle > 0.0f)
            rmse_mforce_perp_angle_array = train_set[batch_id][m].get_rmse_mforce_perp_angle(para, true, m);
        ...
        for (int t = 0; t <= para.num_types; ++t) {
          ...
          fitness_mforce[deviceCount * n + m + t * para.population_size] =
            ...
            + para.lambda_m_para * rmse_mforce_para_array[t]
            + para.lambda_m_perp * rmse_mforce_perp_array[t]
            + para.lambda_m_perp_angle * rmse_mforce_perp_angle_array[t];
        ...
        for (int n = 0; n < population_iter; ++n) {
          ...
          for (int m = 0; m < deviceCount; ++m) {
            ...
            for (int t = 0; t <= para.num_types; ++t) {
              ...
              // mforce (total + para + perp + perp_angle combined)
  ...
  Dataset& dataset)
{
  for (int nc = 0; nc < dataset.Nc; ++nc) {
    ...
    for (int m = 0; m < dataset.structures[nc].num_atom; ++m) {
      ...
      for (int n = 0; n < num_components; ++n) {
        double ref_value = reference[n * dataset.N + offset + m];
        if (n == num_components - 1) {
          fprintf(fid, "%g\n", ref_value);
        } else {
          fprintf(fid, "%g ", ref_value);
...
void Fitness::write_nep_txt(FILE* fid_nep, Parameters& para, float* elite)
{
  ...
  if (is_spin_model) {
    ...
    bool uniform_spin_mref = true;
    for (int t = 1; t < para.num_types; ++t) {
      if (fabs(para.spin_mref[t] - para.spin_mref[0]) >= 1.0e-7f) {
        uniform_spin_mref = false;
        break;
      }
    }
    if (uniform_spin_mref) {
      ...
      for (int t = 0; t < para.num_types; ++t) {
        fprintf(fid_nep, " %g", para.spin_mref[t]);
      }
      fprintf(fid_nep, "\n");
    }
    fprintf(
      fid_nep,
      "spin_2body %d %d\n",
      para.n_max_spin_radial,
      para.basis_size_spin_radial);
    fprintf(
      fid_nep,
      "spin_3body %d %d %d\n",
      para.n_max_spin_angular,
      para.l_max_spin_angular,
      para.basis_size_spin_angular);
  }
  ...
  float* elite)
{
  if (0 == (generation + 1) % 100) {
    ...
    std::vector<float> rmse_mforce_para_train_array;
    std::vector<float> rmse_mforce_perp_train_array;
    ...
    if (has_mforce_train_data) {
      ...
      if (para.lambda_m_para > 0.0f)
        rmse_mforce_para_train_array = train_set[batch_id][0].get_rmse_mforce_para(para, false, 0);
      if (para.lambda_m_perp > 0.0f)
        rmse_mforce_perp_train_array = train_set[batch_id][0].get_rmse_mforce_perp(para, false, 0);
      if (para.lambda_tau > 0.0f) {
    ...
    float rmse_mforce_para_train = rmse_mforce_para_train_array.empty() ? 0.0f : rmse_mforce_para_train_array.back();
    float rmse_mforce_perp_train = rmse_mforce_perp_train_array.empty() ? 0.0f : rmse_mforce_perp_train_array.back();
    ...
    float rmse_mforce_para_test = 0.0f;
    float rmse_mforce_perp_test = 0.0f;
    ...
    if (has_test_set) {
      ...
      std::vector<float> rmse_mforce_para_test_array;
      std::vector<float> rmse_mforce_perp_test_array;
      ...
      if (has_mforce_test_data) {
        ...
        if (para.lambda_m_para > 0.0f)
          rmse_mforce_para_test_array = test_set[0].get_rmse_mforce_para(para, false, 0);
        if (para.lambda_m_perp > 0.0f)
          rmse_mforce_perp_test_array = test_set[0].get_rmse_mforce_perp(para, false, 0);
        if (para.lambda_tau > 0.0f)
      ...
      rmse_mforce_para_test = rmse_mforce_para_test_array.empty() ? 0.0f : rmse_mforce_para_test_array.back();
      rmse_mforce_perp_test = rmse_mforce_perp_test_array.empty() ? 0.0f : rmse_mforce_perp_test_array.back();
    ...
    if (para.train_mode == 0 || para.train_mode == 3) {
      ...
      // Spin mode with mforce labels: report E/F/V and mforce (and optional components).
      const bool show_para_perp = (para.lambda_m_para > 0.0f || para.lambda_m_perp > 0.0f);
      if (show_para_perp) {
        // Extended format: gen loss L1 L2 | E F V M M_para M_perp (train) | E F V M M_para M_perp (test)
        printf(
          "%-8d%-11.5f%-11.5f%-11.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f\n",
          generation + 1,
          loss_total, loss_L1, loss_L2,
          rmse_energy_train, rmse_force_train, rmse_virial_train,
          rmse_mforce_train, rmse_mforce_para_train, rmse_mforce_perp_train,
          rmse_energy_test, rmse_force_test, rmse_virial_test,
          rmse_mforce_test, rmse_mforce_para_test, rmse_mforce_perp_test);
        fprintf(
          fid_loss_out,
          "%-8d%-11.5f%-11.5f%-11.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f\n",
          generation + 1,
          ...
          rmse_energy_test, rmse_force_test, rmse_virial_test,
          rmse_mforce_test, rmse_mforce_para_test, rmse_mforce_perp_test);
      } else if (para.lambda_tau > 0.0f) {
        printf(
          "%-8d%-11.5f%-11.5f%-11.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f\n",
          generation + 1,
          loss_total, loss_L1, loss_L2,
          rmse_energy_train, rmse_force_train, rmse_virial_train,
          rmse_mforce_train, rmse_torque_train,
          rmse_energy_test, rmse_force_test, rmse_virial_test,
          rmse_mforce_test, rmse_torque_test);
        fprintf(
          fid_loss_out,
          "%-8d%-11.5f%-11.5f%-11.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f%-13.5f\n",
          generation + 1,
...
{
  if (dataset.spin_energy.size() == (size_t)dataset.N) {
    dataset.spin_energy.copy_to_host(dataset.spin_energy_cpu.data());
  } else {
  ...
  if (dataset.spin_energy.size() == (size_t)dataset.N) {
    output(false, 1, fid_energy, dataset.spin_energy_cpu.data(), dataset.energy_ref_cpu.data(), dataset);
  } else {
