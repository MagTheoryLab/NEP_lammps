...
int main(int argc, char* argv[])
{
  ...
  printf(
    "Precision mode: StructReal=%s, SpinReal=%s (%s).\n",
    MAIN_NEP_STRUCT_PRECISION_NAME,
    MAIN_NEP_SPIN_PRECISION_NAME,
#ifdef MAIN_NEP_SPIN_DOUBLE
    "MAIN_NEP_SPIN_DOUBLE enabled"
#else
    "MAIN_NEP_SPIN_DOUBLE disabled"
#endif
  );
  print_line_2();
