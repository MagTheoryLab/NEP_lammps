...
void Parameters::set_default_parameters()
{
  ...
  is_lambda_m_para_set = false;
  is_lambda_m_perp_set = false;
  is_lambda_m_perp_angle_set = false;
  ...
  is_spin_onsite_set = false;
  is_spin_2body_set = false;
  is_spin_3body_set = false;


  train_mode = 0;              // potential
  prediction = 0;              // not prediction mode
  num_types = 0;               // populated by parse_type
  ...
  lambda_m_para = 0.0f;        // longitudinal mforce loss disabled by default
  lambda_m_perp = 0.0f;        // transverse mforce loss disabled by default
  lambda_m_perp_angle = 0.0f;  // transverse mforce direction loss disabled by default
  ...
  spin_mref.resize(NUM_ELEMENTS, 1.0f);
  n_max_spin_radial = 1;
  basis_size_spin_radial = basis_size_radial;
  n_max_spin_angular = 3;
  l_max_spin_angular = 3;
  basis_size_spin_angular = basis_size_angular;
...
void Parameters::calculate_parameters()
{
  ...
  if (spin_mode > 0) {
    ...
    if (n_max_spin_radial < 0 || n_max_spin_radial + 1 > MAX_NUM_N) {
      PRINT_INPUT_ERROR("n_max_spin_radial is out of range for compiled MAX_NUM_N.");
    }
    if (basis_size_spin_radial < 0 || basis_size_spin_radial + 1 > MAX_NUM_N) {
      PRINT_INPUT_ERROR("basis_size_spin_radial is out of range for compiled MAX_NUM_N.");
    }
    if (n_max_spin_angular < 0 || n_max_spin_angular + 1 > MAX_NUM_N) {
      PRINT_INPUT_ERROR("n_max_spin_angular is out of range for compiled MAX_NUM_N.");
    }
    if (basis_size_spin_angular < 0 || basis_size_spin_angular + 1 > MAX_NUM_N) {
      PRINT_INPUT_ERROR("basis_size_spin_angular is out of range for compiled MAX_NUM_N.");
    }
    if (l_max_spin_angular < 1 || l_max_spin_angular > 4) {
      PRINT_INPUT_ERROR("l_max_spin_angular should be in [1,4].");
    }
    const int nspin3 = n_max_spin_angular + 1;
    const int block2_core = nspin3 * l_max_spin_angular;
    const int block2_g1 = (l_max_spin_angular >= 2) ? nspin3 : 0;
    const int block2_across = (nspin3 * (nspin3 - 1) / 2) * l_max_spin_angular;
    dim += spin_pmax;
    dim += 4 * (n_max_spin_radial + 1);
    dim += 5 * block2_core + 2 * block2_g1 + block2_across;
  }
  if (train_mode == 3) {
  ...
  // Descriptor parameter count: baseline lattice c + spin-v2 blocks
  int base_desc_vars =
    num_types * num_types *
  ...
  if (spin_mode > 0) {
    num_c_spin_2body =
      num_types * num_types * (n_max_spin_radial + 1) * (basis_size_spin_radial + 1);
    num_c_spin_3body =
      num_types * num_types * (n_max_spin_angular + 1) * (basis_size_spin_angular + 1);
  }

  number_of_variables_descriptor = base_desc_vars + num_c_spin_2body + num_c_spin_3body;
  ...
  if (fine_tune) {
    if (spin_mode > 0) {
      PRINT_INPUT_ERROR("fine_tune is not supported for NEP-Spin v2.");
    }
...
void Parameters::report_inputs()
{
  ...
  if (spin_mode > 0) {
    ...
    if (is_lambda_m_para_set) {
      printf("    (input)   lambda_m_para = %g.\n", lambda_m_para);
    } else if (lambda_m_para > 0.0f) {
      printf("    (default) lambda_m_para = %g.\n", lambda_m_para);
    }
    if (is_lambda_m_perp_set) {
      printf("    (input)   lambda_m_perp = %g.\n", lambda_m_perp);
    } else if (lambda_m_perp > 0.0f) {
      printf("    (default) lambda_m_perp = %g.\n", lambda_m_perp);
    }
    if (is_lambda_m_perp_angle_set) {
      printf("    (input)   lambda_m_perp_angle = %g.\n", lambda_m_perp_angle);
    } else if (lambda_m_perp_angle > 0.0f) {
      printf("    (default) lambda_m_perp_angle = %g.\n", lambda_m_perp_angle);
    }
    ...
    const bool uniform_spin_mref =
      (num_types <= 1) ||
      std::all_of(
        spin_mref.begin() + 1,
        spin_mref.begin() + num_types,
        [&](float value) { return fabs(value - spin_mref[0]) < 1.0e-7f; });
    if (uniform_spin_mref) {
      printf(
        "    (%s)   spin_onsite = pmax %d onsite_basis_mode %d mref %g.\n",
        is_spin_onsite_set ? "input" : "default",
        spin_pmax,
        spin_onsite_basis_mode,
        spin_mref[0]);
    } else {
      printf(
        "    (%s)   spin_onsite = pmax %d onsite_basis_mode %d mref",
        is_spin_onsite_set ? "input" : "default",
        spin_pmax,
        spin_onsite_basis_mode);
      for (int t = 0; t < num_types; ++t) {
        printf(" %s:%g", elements[t].c_str(), spin_mref[t]);
      }
      printf(".\n");
    }
    printf(
      "    (%s)   spin_2body = n_max %d basis_size %d.\n",
      is_spin_2body_set ? "input" : "default",
      n_max_spin_radial,
      basis_size_spin_radial);
    printf(
      "    (%s)   spin_3body = n_max %d l_max %d basis_size %d.\n",
      is_spin_3body_set ? "input" : "default",
      n_max_spin_angular,
      l_max_spin_angular,
      basis_size_spin_angular);
  }
...
void Parameters::parse_one_keyword(std::vector<std::string>& tokens)
{
  ...
  } else if (strcmp(param[0], "lambda_m_para") == 0) {
    parse_lambda_m_para(param, num_param);
  } else if (strcmp(param[0], "lambda_m_perp") == 0) {
    parse_lambda_m_perp(param, num_param);
  } else if (strcmp(param[0], "lambda_m_perp_angle") == 0) {
    parse_lambda_m_perp_angle(param, num_param);
  ...
  } else if (strcmp(param[0], "spin_onsite") == 0) {
    parse_spin_onsite(param, num_param);
  } else if (strcmp(param[0], "spin_2body") == 0) {
    parse_spin_2body(param, num_param);
  } else if (strcmp(param[0], "spin_3body") == 0) {
    parse_spin_3body(param, num_param);
  } else {
...
void Parameters::parse_lambda_m_para(const char** param, int num_param)
{
  is_lambda_m_para_set = true;
  if (num_param != 2) {
    PRINT_INPUT_ERROR("lambda_m_para should have 1 parameter.\n");
  }
  double v = 0.0;
  if (!is_valid_real(param[1], &v)) {
    PRINT_INPUT_ERROR("lambda_m_para should be a number.\n");
  }
  if (v < 0.0) {
    PRINT_INPUT_ERROR("lambda_m_para should be >= 0.\n");
  }
  lambda_m_para = (float)v;
}

void Parameters::parse_lambda_m_perp(const char** param, int num_param)
{
  is_lambda_m_perp_set = true;
  if (num_param != 2) {
    PRINT_INPUT_ERROR("lambda_m_perp should have 1 parameter.\n");
  }
  double v = 0.0;
  if (!is_valid_real(param[1], &v)) {
    PRINT_INPUT_ERROR("lambda_m_perp should be a number.\n");
  }
  if (v < 0.0) {
    PRINT_INPUT_ERROR("lambda_m_perp should be >= 0.\n");
  }
  lambda_m_perp = (float)v;
}

void Parameters::parse_lambda_m_perp_angle(const char** param, int num_param)
{
  is_lambda_m_perp_angle_set = true;
  if (num_param != 2) {
    PRINT_INPUT_ERROR("lambda_m_perp_angle should have 1 parameter.\n");
  }
  double v = 0.0;
  if (!is_valid_real(param[1], &v)) {
    PRINT_INPUT_ERROR("lambda_m_perp_angle should be a number.\n");
  }
  if (v < 0.0) {
    PRINT_INPUT_ERROR("lambda_m_perp_angle should be >= 0.\n");
  }
  lambda_m_perp_angle = (float)v;
}
...
void Parameters::parse_spin_onsite(const char** param, int num_param)
{
  is_spin_onsite_set = true;
  const bool use_single_mref = (num_param == 4);
  const bool use_per_type_mref = (num_types > 0 && num_param == 3 + num_types);
  if (!use_single_mref && !use_per_type_mref) {
    PRINT_INPUT_ERROR(
      "spin_onsite should be: spin_onsite <pmax> <basis_mode> <mref> or "
      "spin_onsite <pmax> <basis_mode> <mref_type1> ... <mref_typeN>.\n");
  }
  if (!is_valid_int(param[1], &spin_pmax)) {
    PRINT_INPUT_ERROR("spin_onsite pmax should be an integer.\n");
  }
  if (spin_pmax < 0 || spin_pmax > 8) {
    PRINT_INPUT_ERROR("spin_onsite pmax should be in [0,8].\n");
  }
  if (!is_valid_int(param[2], &spin_onsite_basis_mode)) {
    PRINT_INPUT_ERROR("spin_onsite basis_mode should be an integer.\n");
  }
  if (spin_onsite_basis_mode < 0 || spin_onsite_basis_mode > 2) {
    PRINT_INPUT_ERROR("spin_onsite basis_mode should be in [0,2].\n");
  }
  if (spin_mref.size() != NUM_ELEMENTS) {
    spin_mref.assign(NUM_ELEMENTS, 1.0f);
  }
  const int mref_count = use_single_mref ? 1 : num_types;
  for (int i = 0; i < mref_count; ++i) {
    double mref_tmp = 0.0;
    if (!is_valid_real(param[3 + i], &mref_tmp)) {
      PRINT_INPUT_ERROR("spin_onsite mref should be a number.\n");
    }
    const float parsed_mref = static_cast<float>(mref_tmp);
    if (!(parsed_mref > 0.0f)) {
      PRINT_INPUT_ERROR("spin_onsite mref should be > 0.\n");
    }
    if (use_single_mref) {
      for (float& value : spin_mref) {
        value = parsed_mref;
      }
      break;
    }
    spin_mref[i] = parsed_mref;
  }
}

void Parameters::parse_spin_2body(const char** param, int num_param)
{
  is_spin_2body_set = true;
  if (num_param != 3) {
    PRINT_INPUT_ERROR("spin_2body should be: spin_2body <n_max_spin_radial> <basis_size_spin_radial>.\n");
  }
  if (!is_valid_int(param[1], &n_max_spin_radial)) {
    PRINT_INPUT_ERROR("spin_2body n_max_spin_radial should be an integer.\n");
  }
  if (!is_valid_int(param[2], &basis_size_spin_radial)) {
    PRINT_INPUT_ERROR("spin_2body basis_size_spin_radial should be an integer.\n");
  }
  if (n_max_spin_radial < 0) {
    PRINT_INPUT_ERROR("spin_2body n_max_spin_radial should be >= 0.\n");
  }
  if (basis_size_spin_radial < 0) {
    PRINT_INPUT_ERROR("spin_2body basis_size_spin_radial should be >= 0.\n");
  }
}

void Parameters::parse_spin_3body(const char** param, int num_param)
{
  is_spin_3body_set = true;
  if (num_param != 4) {
    PRINT_INPUT_ERROR(
      "spin_3body should be: spin_3body <n_max_spin_angular> <l_max_spin_angular> <basis_size_spin_angular>.\n");
  }
  if (!is_valid_int(param[1], &n_max_spin_angular)) {
    PRINT_INPUT_ERROR("spin_3body n_max_spin_angular should be an integer.\n");
  }
  if (!is_valid_int(param[2], &l_max_spin_angular)) {
    PRINT_INPUT_ERROR("spin_3body l_max_spin_angular should be an integer.\n");
  }
  if (!is_valid_int(param[3], &basis_size_spin_angular)) {
    PRINT_INPUT_ERROR("spin_3body basis_size_spin_angular should be an integer.\n");
  }
  if (n_max_spin_angular < 0) {
    PRINT_INPUT_ERROR("spin_3body n_max_spin_angular should be >= 0.\n");
  }
  if (l_max_spin_angular < 1 || l_max_spin_angular > 4) {
    PRINT_INPUT_ERROR("spin_3body l_max_spin_angular should be in [1,4].\n");
  }
  if (basis_size_spin_angular < 0) {
    PRINT_INPUT_ERROR("spin_3body basis_size_spin_angular should be >= 0.\n");
  }
}
...
void Parameters::parse_spin_mode(const char** param, int num_param)
{
  ...
  if (spin_mode < 0 || spin_mode > 1) {
    PRINT_INPUT_ERROR("spin_mode should = 0 or 1.\n");
  }
