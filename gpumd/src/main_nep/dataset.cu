...
const SpinReal* get_energy_prediction_ptr(const Dataset& dataset)
{
  return dataset.spin_energy.size() == static_cast<size_t>(dataset.N) ? dataset.spin_energy.data()
                                                                      : nullptr;
}
...
void Dataset::copy_structures(std::vector<Structure>& structures_input, int n1, int n2)
{
  ...
  for (int n = 0; n < Nc; ++n) {
    ...
    // allocate spin and mforce containers if present
    if (structures_input[n_input].has_spin) {
    ...
    if (structures_input[n_input].has_mforce) {
...
{
  std::vector<StructReal> box_cpu(Nc * 18);
  std::vector<StructReal> box_original_cpu(Nc * 9);
  std::vector<int> num_cell_cpu(Nc * 3);
  std::vector<StructReal> r_cpu(N * 3);
  ...
  // Pre-allocate SoA buffers for optional fields when present
  bool has_any_spin = false;
  bool has_any_mforce = false;
  for (int n = 0; n < Nc; ++n) {
    has_any_spin = has_any_spin || (structures[n].has_spin != 0);
    has_any_mforce = has_any_mforce || (structures[n].has_mforce != 0);
  }
  if (has_any_spin) { spin_cpu.resize(N * 3); }
  if (has_any_mforce) { mforce_ref_cpu.resize(N * 3); }
  ...
  for (int n = 0; n < Nc; ++n) {
    ...
    for (int na = 0; na < structures[n].num_atom; ++na) {
      ...
      if (structures[n].has_spin) {
        int idx = Na_sum_cpu[n] + na;
        spin_cpu[idx] = structures[n].sx[na];
        spin_cpu[idx + N] = structures[n].sy[na];
        spin_cpu[idx + N * 2] = structures[n].sz[na];
      }
      if (structures[n].has_mforce) {
        int idx = Na_sum_cpu[n] + na;
        mforce_ref_cpu[idx] = structures[n].mfx[na];
        mforce_ref_cpu[idx + N] = structures[n].mfy[na];
        mforce_ref_cpu[idx + N * 2] = structures[n].mfz[na];
      }
      if (structures[n].has_atomic_virial) {
  ...
  if (spin_cpu.size() == (size_t)N * 3) {
    spin.resize(N * 3);
    spin.copy_from_host(spin_cpu.data());
    // Allocate predicted magnetic force buffer once; it will be filled during NEP-Spin evaluation.
    mforce.resize(N * 3);
    spin_energy.resize(N);
  }
  if (mforce_ref_cpu.size() == (size_t)N * 3) {
...
// Sum magnetic-force error over only atoms with non-zero spin |s|^2 > eps.
// Also returns the per-configuration count of such atoms.
static __global__ void gpu_sum_mforce_error_nzspin(
  bool use_weight,
  StructReal force_delta,
  int* g_Na,
  int* g_Na_sum,
  int* g_type,
  StructReal* g_type_weight,
  const SpinReal eps,
  const SpinReal* g_sx,
  const SpinReal* g_sy,
  const SpinReal* g_sz,
  const SpinReal* g_mfx,
  const SpinReal* g_mfy,
  const SpinReal* g_mfz,
  const SpinReal* g_mfx_ref,
  const SpinReal* g_mfy_ref,
  const SpinReal* g_mfz_ref,
  StructReal* error_gpu,
  int* nz_count_gpu)
{
  int tid = threadIdx.x;
  int bid = blockIdx.x;
  ...
  double* s_error = sdata;
  double* s_count = sdata + blockDim.x;
  s_error[tid] = 0.0;
  s_count[tid] = 0.0;

  for (int n = N1 + tid; n < N2; n += blockDim.x) {
    const SpinReal sx = g_sx[n];
    const SpinReal sy = g_sy[n];
    const SpinReal sz = g_sz[n];
    const SpinReal s2 = sx * sx + sy * sy + sz * sz;
    if (s2 > eps) {
      const SpinReal rx = g_mfx_ref[n];
      const SpinReal ry = g_mfy_ref[n];
      const SpinReal rz = g_mfz_ref[n];
      const SpinReal dx = g_mfx[n] - rx;
      const SpinReal dy = g_mfy[n] - ry;
      const SpinReal dz = g_mfz[n] - rz;
      double diff_square = (double)dx * dx + (double)dy * dy + (double)dz * dz;
      if (use_weight) {
        const double type_weight = g_type_weight[g_type[n]];
        diff_square *= type_weight * type_weight;
      }
      if (use_weight && force_delta > 0.0f) {
        const double mag = sqrt((double)rx * rx + (double)ry * ry + (double)rz * rz);
        diff_square *= force_delta / (force_delta + mag);
      }
      s_error[tid] += diff_square;
      s_count[tid] += 1.0;
    }
  ...
  if (tid == 0) {
    error_gpu[bid] = (StructReal)s_error[0];
    nz_count_gpu[bid] = (int)(s_count[0] + 0.5f);
  }
}

// Per-atom mforce error projected onto the spin direction (para, component_mode=0)
// or the perpendicular plane (perp, component_mode=1).
// Only atoms with non-zero spin |s|^2 > eps contribute.
static __global__ void gpu_sum_mforce_component_error_nzspin(
  bool use_weight,
  StructReal force_delta,
  int component_mode,
  int* g_Na,
  ...
  int* nz_count_gpu)
{
  ...
  for (int n = N1 + tid; n < N2; n += blockDim.x) {
    ...
    SpinReal sy = g_sy[n];
    SpinReal sz = g_sz[n];
    SpinReal s2 = sx * sx + sy * sy + sz * sz;
    if (s2 > eps) {
      SpinReal rx = g_mfx_ref[n];
      SpinReal ry = g_mfy_ref[n];
      SpinReal rz = g_mfz_ref[n];
      SpinReal dx = g_mfx[n] - rx;
      SpinReal dy = g_mfy[n] - ry;
      SpinReal dz = g_mfz[n] - rz;

      // Project error onto spin direction (para) or onto perpendicular plane (perp).
      SpinReal proj = (dx * sx + dy * sy + dz * sz) / (s2 + 1.0e-12);
      SpinReal px = proj * sx;
      SpinReal py = proj * sy;
      SpinReal pz = proj * sz;
      if (component_mode == 0) {
        // para: keep only the longitudinal part
        dx = px; dy = py; dz = pz;
      } else {
        // perp: keep only the transverse part
        dx -= px; dy -= py; dz -= pz;
      }
...
// Minimum squared magnitude of the reference perp mforce for the angle error to be counted.
static constexpr StructReal kMforcePerpAngleRef2Min = 1.0e-4f;

// Per-atom angular error between predicted and reference transverse mforce vectors.
// Measures (1 - cos θ) where θ is the angle between the two perp projections.
// Skips atoms where the reference perp magnitude is below kMforcePerpAngleRef2Min.
static __global__ void gpu_sum_mforce_perp_angle_error_nzspin(
  bool use_weight,
  StructReal perp_threshold,
  int* g_Na,
  int* g_Na_sum,
  int* g_type,
  StructReal* g_type_weight,
  ...
  int* nz_count_gpu)
{
  ...
  for (int n = N1 + tid; n < N2; n += blockDim.x) {
    ...
    const SpinReal inv_s2 = 1.0 / (s2 + 1.0e-12);
    SpinReal px = g_mfx[n];
    SpinReal py = g_mfy[n];
    SpinReal pz = g_mfz[n];
    SpinReal rx = g_mfx_ref[n];
    ...
    // Project both vectors onto the perpendicular plane.
    SpinReal proj_p = (px * sx + py * sy + pz * sz) * inv_s2;
    SpinReal proj_r = (rx * sx + ry * sy + rz * sz) * inv_s2;
    SpinReal ppx = px - proj_p * sx;
    SpinReal ppy = py - proj_p * sy;
    SpinReal ppz = pz - proj_p * sz;
    SpinReal rpx = rx - proj_r * sx;
    SpinReal rpy = ry - proj_r * sy;
    SpinReal rpz = rz - proj_r * sz;

    SpinReal rp2 = rpx * rpx + rpy * rpy + rpz * rpz;
    if (rp2 <= perp_threshold) continue; // skip near-zero reference perp

    SpinReal pp2 = ppx * ppx + ppy * ppy + ppz * ppz;
    SpinReal dot = ppx * rpx + ppy * rpy + ppz * rpz;
    SpinReal cos_theta = dot / (sqrt((double)pp2 * rp2) + 1.0e-12);
    if (cos_theta > SpinReal(1.0)) cos_theta = SpinReal(1.0);
    if (cos_theta < SpinReal(-1.0)) cos_theta = SpinReal(-1.0);
    double angle_err = 1.0 - cos_theta;
    if (use_weight) {
      const double type_weight = g_type_weight[g_type[n]];
      angle_err *= type_weight * type_weight;
    }
    s_error[tid] += angle_err;
    s_count[tid] += 1.0;
  }
  __syncthreads();
  ...
  for (int n = N1 + tid; n < N2; n += blockDim.x) {
    ...
    if (s2 > eps) {
      ...
      if (use_weight) {
        ...
        if (force_delta > 0.0f) {
          const double mag = sqrt((double)rx * rx + (double)ry * ry + (double)rz * rz);
          const double fac = force_delta / (force_delta + mag);
          if (fac > 0.0f) {
            w *= sqrt(fac);
          }
        }
...
std::vector<float> Dataset::get_rmse_mforce(Parameters& para, const bool use_weight, int device_id)
{
  CHECK(gpuSetDevice(device_id));
  const int block_size = 256;
  std::vector<float> rmse_array(para.num_types + 1, 0.0f);
  ...
  // If there is no reference mforce (or no predicted buffer), mforce loss is undefined;
  // return zeros so callers can safely include this term conditionally.
  ...
  if (has_spin) {
    ...
    gpu_sum_mforce_error_nzspin<<<Nc, block_size, sizeof(double) * block_size * 2>>>(
      use_weight,
      para.force_delta,
      Na.data(),
      Na_sum.data(),
      ...
      error_gpu.data(),
      nz_count_gpu.data());
    int mem_err = sizeof(StructReal) * Nc;
    CHECK(gpuMemcpy(error_cpu.data(), error_gpu.data(), mem_err, gpuMemcpyDeviceToHost));
    std::vector<int> nz_spin_count_per_conf(Nc, 0);
    CHECK(gpuMemcpy(nz_spin_count_per_conf.data(), nz_count_gpu.data(), sizeof(int) * Nc, gpuMemcpyDeviceToHost));

    for (int n = 0; n < Nc; ++n) {
      int eff_count = nz_spin_count_per_conf[n];
      if (eff_count <= 0) { continue; }
      StructReal rmse_temp = use_weight ? weight_cpu[n] * weight_cpu[n] * error_cpu[n] : error_cpu[n];
      for (int t = 0; t < para.num_types + 1; ++t) {
        if (has_type[t * Nc + n]) {
          rmse_array[t] += rmse_temp;
          count_array[t] += eff_count;
        }
      }
    }
  }
...
// Shared helper for para (component_mode=0) and perp (component_mode=1) mforce RMSE.
static std::vector<float> get_rmse_mforce_component_impl(
  Dataset& dataset, Parameters& para, const bool use_weight, int device_id, int component_mode)
{
  CHECK(gpuSetDevice(device_id));
  const int block_size = 256;
  std::vector<float> rmse_array(para.num_types + 1, 0.0f);
  ...
  if (dataset.mforce_ref_gpu.size() != (size_t)dataset.N * 3 ||
      dataset.mforce.size() != (size_t)dataset.N * 3) {
    return rmse_array;
  }
  const bool has_spin = (dataset.spin_cpu.size() == (size_t)dataset.N * 3);
  if (!has_spin) return rmse_array;

  GPU_Vector<int> nz_count_gpu;
  nz_count_gpu.resize(dataset.Nc);
  gpu_sum_mforce_component_error_nzspin<<<dataset.Nc, block_size, sizeof(double) * block_size * 2>>>(
    use_weight,
    para.force_delta,
    component_mode,
    dataset.Na.data(),
    dataset.Na_sum.data(),
    dataset.type.data(),
    dataset.type_weight_gpu.data(),
    1.0e-8,
    dataset.spin.data(),
    dataset.spin.data() + dataset.N,
    dataset.spin.data() + dataset.N * 2,
    dataset.mforce.data(),
    dataset.mforce.data() + dataset.N,
    dataset.mforce.data() + dataset.N * 2,
    dataset.mforce_ref_gpu.data(),
    dataset.mforce_ref_gpu.data() + dataset.N,
    dataset.mforce_ref_gpu.data() + dataset.N * 2,
    dataset.error_gpu.data(),
    nz_count_gpu.data());
  CHECK(gpuMemcpy(
    dataset.error_cpu.data(), dataset.error_gpu.data(),
    sizeof(StructReal) * dataset.Nc, gpuMemcpyDeviceToHost));
  std::vector<int> nz_count(dataset.Nc, 0);
  CHECK(gpuMemcpy(
    nz_count.data(), nz_count_gpu.data(),
    sizeof(int) * dataset.Nc, gpuMemcpyDeviceToHost));

  for (int n = 0; n < dataset.Nc; ++n) {
    int eff_count = nz_count[n];
    if (eff_count <= 0) continue;
    StructReal rmse_temp = use_weight
      ? dataset.weight_cpu[n] * dataset.weight_cpu[n] * dataset.error_cpu[n]
      : dataset.error_cpu[n];
    for (int t = 0; t <= para.num_types; ++t) {
      if (dataset.has_type[t * dataset.Nc + n]) {
        rmse_array[t] += rmse_temp;
        count_array[t] += eff_count;
      }
    }
  ...
  for (int t = 0; t <= para.num_types; ++t) {
    if (count_array[t] > 0) {
      rmse_array[t] = sqrtf(rmse_array[t] / (count_array[t] * 3));
    }
  }
  return rmse_array;
}

std::vector<float> Dataset::get_rmse_mforce_para(
  Parameters& para, const bool use_weight, int device_id)
{
  return get_rmse_mforce_component_impl(*this, para, use_weight, device_id, 0);
}

std::vector<float> Dataset::get_rmse_mforce_perp(
  Parameters& para, const bool use_weight, int device_id)
{
  return get_rmse_mforce_component_impl(*this, para, use_weight, device_id, 1);
}

std::vector<float> Dataset::get_rmse_mforce_perp_angle(
  Parameters& para, const bool use_weight, int device_id)
{
  CHECK(gpuSetDevice(device_id));
  const int block_size = 256;
  std::vector<float> angle_array(para.num_types + 1, 0.0f);
  std::vector<int> count_array(para.num_types + 1, 0);

  if (mforce_ref_gpu.size() != (size_t)N * 3 || mforce.size() != (size_t)N * 3) {
    return angle_array;
  }
  if (spin_cpu.size() != (size_t)N * 3) return angle_array;

  GPU_Vector<int> nz_count_gpu;
  nz_count_gpu.resize(Nc);
  gpu_sum_mforce_perp_angle_error_nzspin<<<Nc, block_size, sizeof(double) * block_size * 2>>>(
    use_weight,
    kMforcePerpAngleRef2Min,
    Na.data(),
    Na_sum.data(),
    type.data(),
    type_weight_gpu.data(),
    ...
    nz_count_gpu.data());
  CHECK(gpuMemcpy(
    error_cpu.data(), error_gpu.data(), sizeof(StructReal) * Nc, gpuMemcpyDeviceToHost));
  std::vector<int> nz_count(Nc, 0);
  CHECK(gpuMemcpy(
    nz_count.data(), nz_count_gpu.data(), sizeof(int) * Nc, gpuMemcpyDeviceToHost));

  for (int n = 0; n < Nc; ++n) {
    if (nz_count[n] <= 0) continue;
    StructReal err = use_weight
      ? weight_cpu[n] * weight_cpu[n] * error_cpu[n]
      : error_cpu[n];
    for (int t = 0; t <= para.num_types; ++t) {
      if (has_type[t * Nc + n]) {
        angle_array[t] += err;
        count_array[t] += nz_count[n];
      }
    }
  }
  for (int t = 0; t <= para.num_types; ++t) {
    if (count_array[t] > 0) {
      angle_array[t] = angle_array[t] / count_array[t]; // mean (1 - cos θ), not RMSE
    }
  }
  return angle_array;
}
...
gpu_get_energy_shift(
  ...
  const EnergyT* g_pe, 
  StructReal* g_pe_ref, 
  StructReal* g_pe_weight, 
  StructReal* g_energy_shift)
{
  int tid = threadIdx.x;
  int bid = blockIdx.x;
  int Na = g_Na[bid];
  ...
  if (tid == 0) {
    ...
    g_energy_shift[bid] = (StructReal)(diff * (double)g_pe_weight[bid]);
  }
}

template <typename EnergyT>
static __global__ void gpu_sum_pe_error(
  StructReal energy_shift, 
  int* g_Na, 
  int* g_Na_sum, 
  const EnergyT* g_pe, 
  StructReal* g_pe_ref, 
  ...
  StructReal* error_gpu)
{
  ...
  if (tid == 0) {
    ...
    error_gpu[bid] = (StructReal)(diff * diff * (double)g_pe_weight[bid]);
  }
}
  ...
  int device_id)
{
  ...
  const int block_size = 256;
  int mem = sizeof(StructReal) * Nc;
  const SpinReal* energy_prediction = get_energy_prediction_ptr(*this);
  const bool use_spin_energy = (energy_prediction != nullptr);
