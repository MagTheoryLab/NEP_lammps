...
void PairNEPGPUKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  ...
  if (want_vatom_buffer) {
    ...
    if (vflag_atom && !vatom) error->one(FLERR, "NEP_GPU/kk: vflag_atom set but vatom is null (internal allocation mismatch)");
    if (cvflag_atom && !cvatom) error->one(FLERR, "NEP_GPU/kk: cvflag_atom set but cvatom is null (internal allocation mismatch)");
    if (vflag_atom && vatom) {
