...
void PairNEPSpinGPUKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  ...
  if (want_vatom_buffer) {
    ...
    // Standard per-atom virial: vatom[i][0..5] (xx,yy,zz,xy,xz,yz)
    if (vflag_atom && !vatom) error->one(FLERR, "NEP_SPIN_GPU/kk: vflag_atom set but vatom is null (internal allocation mismatch)");
    if (cvflag_atom && !cvatom) error->one(FLERR, "NEP_SPIN_GPU/kk: cvflag_atom set but cvatom is null (internal allocation mismatch)");
    if (vflag_atom && vatom) {
    ...
    // Centroid stress compute: cvatom[i][0..8] (xx..zy)
