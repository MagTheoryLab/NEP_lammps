...
struct NepGpuSpinResult {
  ...
  // Optional analysis channel: interface-local mforce accumulated from neighbors
  // on the left side (x_j < iface_x and |x_i - iface_x| <= iface_half_width).
  double* fm_left_iface = nullptr; // host AoS, length 3*natoms (optional)
  double iface_x = 0.0;
  double iface_half_width = 0.0;
...
struct NepGpuSpinDeviceResult {
  ...
  // Optional analysis channel: interface-local mforce accumulated from neighbors
  // on the left side (x_j < iface_x and |x_i - iface_x| <= iface_half_width).
  double* fm_left_iface = nullptr; // device AoS, length 3*natoms (accumulated into)
  double iface_x = 0.0;
  double iface_half_width = 0.0;
...
private:
  ...
  double* d_fm_left_iface_ = nullptr;
