  ...
  ForceT* g_fz)
{
  ...
  fi_y += f12[1];
  fi_z += f12[2];
  atomicAdd(&g_fx[n2], static_cast<ForceT>(-f12[0]));
  atomicAdd(&g_fy[n2], static_cast<ForceT>(-f12[1]));
  atomicAdd(&g_fz[n2], static_cast<ForceT>(-f12[2]));
  v_xx -= r12[0] * f12[0];
  v_yy -= r12[1] * f12[1];
  v_zz -= r12[2] * f12[2];
  v_xy -= r12[0] * f12[1];
...
static __device__ __forceinline__ bool should_accumulate_left_iface(
  const bool enable,
  const float xi,
  const float xj,
  const float iface_x,
  const float iface_half_width)
{
  if (!enable) return false;
  return (fabsf(xi - iface_x) <= iface_half_width) && (xj < iface_x);
}
  ...
  double* g_mz_left)
{
  ...
  float mi_left_x = 0.0f, mi_left_y = 0.0f, mi_left_z = 0.0f;
  const bool enable_left_iface = (g_mx_left != nullptr) && (iface_half_width > 0.0f);
  const float xi = static_cast<float>(g_x[n1]);
  int bs = paramb.basis_size_spin_radial;
  if (bs >= MAX_NUM_N) bs = MAX_NUM_N - 1;
  for (int i1 = 0; i1 < neighbor_number; ++i1) {
    const int index = i1 * N + n1;
    ...
    const int t2 = g_type[n2];
    const float r12[3] = {g_x12[index], g_y12[index], g_z12[index]};
    const float xj = xi + r12[0];
    const bool take_left_iface = should_accumulate_left_iface(enable_left_iface, xi, xj, iface_x, iface_half_width);
    const float d12 = sqrtf(r12[0] * r12[0] + r12[1] * r12[1] + r12[2] * r12[2]);
    if (!(d12 > 0.0f)) continue;
    const float inv_d12 = 1.0f / d12;
    const float rhat[3] = {r12[0] * inv_d12, r12[1] * inv_d12, r12[2] * inv_d12};
    ...
    float mi_pair_x = 0.0f, mi_pair_y = 0.0f, mi_pair_z = 0.0f;
    for (int n = 0; n < nspin; ++n) {
      ...
      const float dmi_x = msign * (Fex * gn * sj[0] + Fdmi * gn * (sj[1] * rhat[2] - sj[2] * rhat[1]) +
                                   Fani * gn * sj_dot_r * rhat[0] + Fsia * gn * 2.0f * si_dot_r * rhat[0]);
      const float dmi_y = msign * (Fex * gn * sj[1] + Fdmi * gn * (sj[2] * rhat[0] - sj[0] * rhat[2]) +
                                   Fani * gn * sj_dot_r * rhat[1] + Fsia * gn * 2.0f * si_dot_r * rhat[1]);
      const float dmi_z = msign * (Fex * gn * sj[2] + Fdmi * gn * (sj[0] * rhat[1] - sj[1] * rhat[0]) +
  ...
  if (enable_left_iface) {
    atomic_add_force(&g_mx_left[n1], static_cast<double>(mi_left_x));
    atomic_add_force(&g_my_left[n1], static_cast<double>(mi_left_y));
    atomic_add_force(&g_mz_left[n1], static_cast<double>(mi_left_z));
  }
  ...
  double* g_mz_left)
{
  ...
  for (int n = 0; n < nspin3; ++n) {
    ...
    for (int i1 = 0; i1 < neighbor_number; ++i1) {
      ...
      const float dmi_x = msign * gn * projc * sj[0] + msign * gn * projD * cross_sj_r[0];
      const float dmi_y = msign * gn * projc * sj[1] + msign * gn * projD * cross_sj_r[1];
      const float dmi_z = msign * gn * projc * sj[2] + msign * gn * projD * cross_sj_r[2];
      mi_x += dmi_x;
      mi_y += dmi_y;
      mi_z += dmi_z;
      if (take_left_iface) {
...
void NEP_Spin_LMP::ensure_capacity(int natoms)
{
  ...
  mforce_left_iface_soa_.resize(static_cast<size_t>(natoms) * 3);
  ...
  double* virial_raw9_out)
{
  ...
  auto reset_state = [&]() {
    ...
    if (fm_left_iface_aos_dev) {
      zero_mforce_spin<<<grid, block, 0, st>>>(
        natoms,
        mforce_left_iface_soa_.data(),
        mforce_left_iface_soa_.data() + natoms,
        mforce_left_iface_soa_.data() + 2 * natoms);
    }
    GPU_CHECK_KERNEL
  };
  ...
  auto run_backward = [&]() {
    ...
    time_region_ms("force_spin_2body", [&]() {
      find_force_spin_2body<<<grid_size, BLOCK_SIZE, 0, st>>>(
        ...
        mforce_soa_.data() + natoms,
        mforce_soa_.data() + 2 * natoms,
        static_cast<float>(iface_x),
        static_cast<float>(iface_half_width),
        fm_left_iface_aos_dev ? mforce_left_iface_soa_.data() : nullptr,
        fm_left_iface_aos_dev ? mforce_left_iface_soa_.data() + natoms : nullptr,
        fm_left_iface_aos_dev ? mforce_left_iface_soa_.data() + 2 * natoms : nullptr);
      GPU_CHECK_KERNEL
    });
  ...
  if (fm_left_iface_aos_dev) {
    time_region_ms("scatter_mforce_left_iface", [&]() {
      const int block = 256;
      const int grid = (natoms + block - 1) / block;
      scatter_mforce_soa_to_fm_aos_add_scale<<<grid, block, 0, st>>>(
        natoms,
