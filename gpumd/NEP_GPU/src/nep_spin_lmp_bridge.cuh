...
public:
  ...
  // - Optional `fm_left_iface_aos_dev` collects left-side interface contributions.
  void compute_with_neighbors_device(
    ...
    double* fm_aos_dev,
    double* fm_left_iface_aos_dev, // optional AoS length 3*natoms
    double iface_x,                // interface plane x = iface_x
    double iface_half_width,       // interface band half-width around iface_x
    double inv_hbar,
...
private:
  ...
  GPU_Vector<double> mforce_left_iface_soa_; // 3*natoms (SoA, optional analysis channel)
