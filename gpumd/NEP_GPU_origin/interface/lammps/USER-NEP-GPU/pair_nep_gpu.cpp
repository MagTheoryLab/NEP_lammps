...
void PairNEPGPU::compute(int eflag, int vflag)
{
  ...
  if (vflag_atom && !vatom && !cvatom) {
    error->one(FLERR, "NEP_GPU: per-atom virial requested but neither vatom nor cvatom is allocated");
  }
  if (cvflag_atom && !cvatom && !vatom) {
    error->one(FLERR, "NEP_GPU: centroid per-atom virial requested but neither cvatom nor vatom is allocated");
  }
  if (vatom) {
    for (int i = 0; i < nlocal; ++i) {
      const int idx = 9 * i;
      ...
      vatom[i][3] += 0.5 * (vatom_host[idx + 3] + vatom_host[idx + 6]);
      vatom[i][4] += 0.5 * (vatom_host[idx + 4] + vatom_host[idx + 7]);
      vatom[i][5] += 0.5 * (vatom_host[idx + 5] + vatom_host[idx + 8]);
    }
  }
