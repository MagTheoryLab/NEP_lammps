...
void PairNEPGPU::compute(int eflag, int vflag)
{
  ...
  // Build NepGpuSystem from LAMMPS atom data (local atoms only).
  const int nlocal = atom->nlocal;
  if (nlocal <= 0) return;

  type_host.resize(nlocal);
  xyz_host.resize(3 * nlocal);
  f_host.resize(3 * nlocal);
  eatom_host.resize(nlocal);
  vatom_host.resize(6 * nlocal);
  ...
  // Construct a Box matrix from LAMMPS domain.
  // Map LAMMPS triclinic parameters (h[0..5]) to GPUMD's 3x3 box matrix
  // cpu_h in column-major form:
  //   a = (xprd, 0,    0   )
  //   b = (xy,   yprd, 0   )
  //   c = (xz,   yz,   zprd)
  double h[9];
