...
## Spin-dependent potentials (E(R,S))

If your ML potential depends on both atomic positions and spins, you can request spin-field output via:

- `pair_style mliap ... spin yes`

This requires `atom_style spin` (so `atom->sp`/`atom->fm` are allocated) and a descriptor implementation that overrides
`MLIAPDescriptor::supports_spin()` and `MLIAPDescriptor::compute_spin_forces()` to accumulate the effective field into
`atom->fm`.

If the `spin` keyword is omitted (or set to `spin auto`), spin-field output is enabled automatically when using
`atom_style spin`.

For `pair_style mliap unified`, Python implementations can also directly accumulate the field via the unified data
interface (without using `compute_spin_forces()`).
