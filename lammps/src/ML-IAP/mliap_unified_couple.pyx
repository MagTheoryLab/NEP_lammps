...
cdef extern from "mliap_unified.h" namespace "LAMMPS_NS":
    ...
    cdef void update_spin_fields(MLIAPData *, double *) except +
...
cdef class MLIAPDataPy:
    ...
    def update_pair_forces_gpu(self, fij):
        raise NotImplementedError(
            "MLIAPDataPy.update_pair_forces_gpu() is only available with the KOKKOS unified coupling. "
            "Use 'pair_style mliap/kk unified ...' (and run LAMMPS with '-k on -sf kk')."
        )

    def forward_exchange(self, copy_from, copy_to, vec_len):
        raise NotImplementedError(
            "MLIAPDataPy.forward_exchange() is only available with the KOKKOS unified coupling. "
            "Message-passing ML models (e.g. TACE) require forward/reverse exchange to run in parallel. "
            "Use 'pair_style mliap/kk unified ...' (and run LAMMPS with '-k on -sf kk')."
        )

    def reverse_exchange(self, copy_from, copy_to, vec_len):
        raise NotImplementedError(
            "MLIAPDataPy.reverse_exchange() is only available with the KOKKOS unified coupling. "
            "Message-passing ML models (e.g. TACE) require forward/reverse exchange to run in parallel. "
            "Use 'pair_style mliap/kk unified ...' (and run LAMMPS with '-k on -sf kk')."
        )
    ...
    def sp(self):
        if self.data.sp is NULL:
            return None
        return np.asarray(<double[:self.ntotal, :4]> &self.data.sp[0][0])

    def update_spin_fields(self, h):
        cdef double[:, ::1] h_view = h
        update_spin_fields(self.data, &h_view[0][0])
