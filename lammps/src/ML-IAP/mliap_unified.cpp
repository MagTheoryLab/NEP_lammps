...
/* ----------------------------------------------------------------------
   ...
   passed in as a packed array of length 3*ntotal
------------------------------------------------------------------------- */

void LAMMPS_NS::update_spin_fields(MLIAPData *data, double *h)
{
  if (!data->fm) {
    data->get_error()->all(FLERR, "MLIAP unified: atom->fm is not allocated (need atom_style spin)");
  }
