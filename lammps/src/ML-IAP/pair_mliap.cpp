...
void PairMLIAP::compute(int eflag, int vflag)
{
  ...
  if (spin_enabled) descriptor->compute_spin_forces(data);
  e_tally(data);
...
void PairMLIAP::settings(int narg, char ** arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"model") == 0) {
      ...
      if (iarg + 2 > narg) utils::missing_cmd_args(FLERR, "pair_style mliap spin", error);
      if (strcmp(arg[iarg + 1], "auto") == 0) {
        // Resolve in init_style() once atom_style is known.
        spin_user_specified = false;
        spin_enabled = false;
      } else {
        spin_enabled = utils::logical(FLERR, arg[iarg + 1], false, lmp);
        spin_user_specified = true;
      }
      iarg += 2;
    } else
...
void PairMLIAP::init_style()
{
  ...
  // Auto-enable spin field output when using atom_style spin, unless user
  // explicitly overrode it via "spin yes/no".
  if (!spin_user_specified) spin_enabled = atom->sp_flag;

  if (spin_enabled) {
    if (!atom->sp_flag)
      error->all(FLERR, Error::NOLASTLINE,
                 "pair_style mliap spin requires atom_style spin (sp/fm allocated)");
  }
