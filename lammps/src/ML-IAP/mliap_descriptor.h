...
class MLIAPDescriptor : virtual protected Pointers {
 public:
  ...
  // Optional: spin-dependent potentials E(R,S) can also provide an effective
  // field H = -dE/dM (stored into atom->fm by the descriptor implementation).
  // Base implementation provides no spin support.
  virtual bool supports_spin() const { return false; }
  virtual void compute_spin_forces(class MLIAPData *) {}
