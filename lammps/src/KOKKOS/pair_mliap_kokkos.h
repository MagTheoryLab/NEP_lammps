...
public:
  ...
  // Mark/sync spin field (atom->fm / AtomKokkos::k_fm) after external device writes.
  // Needed when Python/ML code updates `k_fm` through a raw device pointer; otherwise
  // host-side `compute property/atom fmx/fmy/fmz` and `dump` keep seeing zeros.
  void sync_fm_to_host();
