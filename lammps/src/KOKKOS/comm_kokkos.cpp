...
void CommKokkos::init()
{
  ...
  // If reverse communication involves fields beyond f[3] (comm_f_only==0),
  // we can only use device-side reverse comm when the active AtomVecKokkos
  // provides Kokkos pack/unpack kernels for those extra reverse fields.
  if (!comm_f_only && !atomKK->avecKK->reverse_comm_kokkos_extra_flag) {
    ...
    if (sendproc[iswap] != me) {
      ...
      if (sendnum[iswap]) {
        ...
        n = atomKK->avecKK->pack_reverse_self(sendnum[iswap],k_sendlist_iswap,
                                  firstrecv[iswap]);
        // Reverse communication must process swaps sequentially (outer ghost layers
        // accumulate into inner layers).  For device execution, the self-swap path
        // launches kernels; without a fence here, consecutive self swaps can overlap
        // and produce incorrect results, showing up as MPI-layout dependent drift.
        DeviceType().fence();
      }
    }
  }
