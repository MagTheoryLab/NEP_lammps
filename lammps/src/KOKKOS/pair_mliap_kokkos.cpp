...
void PairMLIAPKokkos<DeviceType>::settings(int narg, char ** arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"model") == 0) {
      ...
      // optional 3rd argument: ghost neighborlist boolean
      // If it is not a boolean token, treat it as another keyword (e.g. "spin yes")
      ghostneigh = 0;
      if (iarg + 3 <= narg) {
        const std::string next(arg[iarg + 2]);
        if ((next == "yes") || (next == "on") || (next == "true") || (next == "1") ||
            (next == "no")  || (next == "off")|| (next == "false")|| (next == "0")) {
...
/* ----------------------------------------------------------------------
   Mark device fm modified and sync to host for host-side output.
   This is needed when external code (e.g. Python/ML) writes into AtomKokkos::k_fm
   through a raw device pointer.
------------------------------------------------------------------------- */

template <class DeviceType>
void PairMLIAPKokkos<DeviceType>::sync_fm_to_host()
{
