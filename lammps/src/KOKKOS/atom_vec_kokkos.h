...
class AtomVecKokkos : virtual public AtomVec {
 public:
  ...
  // Some atom styles exchange more than just f[3] in reverse comm (e.g. SPIN
  // adds fm/fm_long).  CommKokkos can only use device-side reverse comm when
  // the active AtomVecKokkos provides pack/unpack kernels for those extra
  // fields; otherwise it must fall back to classic (host) reverse comm.
  int reverse_comm_kokkos_extra_flag;
