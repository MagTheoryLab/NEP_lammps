...
/* ----------------------------------------------------------------------
   set spin fields for owned + ghost atoms on this proc
   h_i = effective field, typically h_i = -dE/dM_i
   passed in as a packed array of length 3*ntotal (on host or device)
------------------------------------------------------------------------- */

void LAMMPS_NS::update_spin_fields(MLIAPDataKokkosDevice *data, double *h)
{
  if (!data->fm) return;

  auto *fm = data->fm;
  const int ntotal = data->ntotal;

  Kokkos::parallel_for(ntotal, KOKKOS_LAMBDA(int i) {
    const int i3 = 3 * i;
    fm[i3 + 0] += h[i3 + 0];
    fm[i3 + 1] += h[i3 + 1];
    fm[i3 + 2] += h[i3 + 2];
  ...
  // `atom->fm` in LAMMPS (used by `compute property/atom fmx/fmy/fmz` and dump)
  // lives on host. Here we update `k_fm` directly on device through a raw pointer,
  // so we must mark the device view modified and sync back to host, otherwise
  // host-side dumps will keep showing zeros.
  Kokkos::fence();
  if (data->pairmliap) data->pairmliap->sync_fm_to_host();
}
