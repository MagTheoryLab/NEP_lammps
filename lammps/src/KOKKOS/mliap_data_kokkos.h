...
public:
  ...
  MLIAPDataKokkosDevice(MLIAPDataKokkos<LMPDeviceType> &base) :
    ...
    numneighs(base.k_numneighs.d_view.data()),
    ...
    eflag(base.eflag),
    vflag(base.vflag),
    // Prefer the explicitly provided Kokkos pair pointer (no RTTI dependency and
    // works for /kk/host as well). This is used by the Python/ML callbacks to
    // trigger required host/device syncs after raw device writes.
    pairmliap(base.k_pairmliap),
#if defined(KOKKOS_ENABLE_CUDA)
    dev(1),
    device_id([]() {
      int id = 0;
  ...
  // Read-only: atom coordinates (owned + ghosts), shape (ntotal, 3)
  double *x;
