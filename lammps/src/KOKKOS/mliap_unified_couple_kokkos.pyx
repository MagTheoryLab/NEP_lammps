...
cdef extern from "mliap_unified_kokkos.h" namespace "LAMMPS_NS":
    ...
    cdef void update_spin_fields(MLIAPDataKokkosDevice *, double *) except +
...
cdef create_array(device, void *pointer, shape,is_int):
    ...
    if (device == 1):
        if cupy is None:
            raise ImportError(
                "LAMMPS ML-IAP Kokkos unified coupling requires CuPy to expose device arrays to Python. "
                "Install a matching build (e.g. 'pip install cupy-cuda12x') or run with Kokkos on host."
            )
        dtype = cupy.double
        itemsize = 8
        if is_int:
            dtype = cupy.int32
            itemsize = 4
        nbytes = int(n_elem * itemsize)
        mem = cupy.cuda.UnownedMemory(ptr=int(<uintptr_t> pointer), owner=None, size=nbytes)
        memptr = cupy.cuda.MemoryPointer(mem, 0)
        return cupy.ndarray(shape, dtype, memptr=memptr)
    else:
        if (len(shape) == 1 ):
            if (is_int):
...
cdef class MLIAPDataPy:
    ...
    def update_pair_forces_gpu(self, fij):
        ...
        try:
            ptr = fij.data.ptr
            if fij.dtype == cupy.float64:
                update_pair_forces(self.data, <double*>ptr)
            else:
                raise TypeError(f"Unsupported fij dtype: ({fij.dtype}); only float64 is supported")
        except Exception:
            ptr = fij.data_ptr()
            if fij.dtype == torch.float64:
                update_pair_forces(self.data, <double*>ptr)
            else:
                raise TypeError(f"Unsupported fij dtype: ({fij.dtype}); only float64 is supported")
    ...
    def update_spin_fields_cpu(self, h):
        cdef double[:, ::1] h_arr
        try:
            h_arr = h
        except:
            h_arr = h.detach().numpy().astype(np.double)
        update_spin_fields(self.data, &h_arr[0][0])

    def update_spin_fields_gpu(self, h):
        cdef uintptr_t ptr
        try:
            ptr = h.data.ptr
            if h.dtype == cupy.float64:
                update_spin_fields(self.data, <double*>ptr)
            else:
                raise TypeError(f"Unsupported h dtype: ({h.dtype}); only float64 is supported")
        except Exception:
            ptr = h.data_ptr()
    ...
    def update_spin_fields(self, h):
        if self.data.dev==0:
            self.update_spin_fields_cpu(h)
        else:
            self.update_spin_fields_gpu(h)
    ...
    def forward_exchange(self, copy_from, copy_to, vec_len):
        ...
        # PyTorch path (GPU-aware, required for Kokkos device runs)
        try:
        ...
        if is_torch:
            if self.data.dev == 1:
                if not getattr(copy_from, "is_cuda", False):
                    raise RuntimeError(
                        "MLIAP Kokkos unified coupling is running on CUDA (dev=1), but forward_exchange() "
                        "received CPU tensors. This will crash with cudaErrorIllegalAddress."
                    )
                idx = None
                try:
                    idx = int(copy_from.device.index)
                except Exception:
                    idx = None
                if idx is not None and idx != int(self.data.device_id):
                    raise RuntimeError(
                        f"MLIAP Kokkos device mismatch: lammps device_id={int(self.data.device_id)} "
                        f"but torch tensor is on cuda:{idx}. Set torch/cupy device to match."
                    )
    ...
    def reverse_exchange(self, copy_from, copy_to, vec_len):
        ...
        if is_torch:
            if self.data.dev == 1:
                if not getattr(copy_from, "is_cuda", False):
                    raise RuntimeError(
                        "MLIAP Kokkos unified coupling is running on CUDA (dev=1), but reverse_exchange() "
                        "received CPU tensors. This will crash with cudaErrorIllegalAddress."
                    )
                idx = None
                try:
                ...
                if idx is not None and idx != int(self.data.device_id):
                    raise RuntimeError(
                        f"MLIAP Kokkos device mismatch: lammps device_id={int(self.data.device_id)} "
                        f"but torch tensor is on cuda:{idx}."
                    )
    ...
    def x(self):
        if self.data.x is NULL:
            return None
        return create_array(self.data.dev, self.data.x, [self.ntotal, 3], False)

    @property
    def sp(self):
        if self.data.sp is NULL:
            return None
        return create_array(self.data.dev, self.data.sp, [self.ntotal, 4], False)
    ...
    @property
    def nghost(self):
        # Number of ghost atoms currently allocated on this rank.
        # Note: with periodic BCs and large cutoffs, this can be > 0 even with a 1x1x1 MPI grid.
        return self.data.ntotal - self.data.nlocal
    ...
    def numneighs(self):
        if self.data.numneighs is NULL:
            return None
        return create_array(self.data.dev, self.data.numneighs, [self.natomneigh], True)
