...
void MLIAPDataKokkos<DeviceType>::generate_neighdata(class NeighList *list_in, int eflag_in, int vflag_in) {
  ...
  unsigned int sync_mask = X_MASK | V_MASK | F_MASK | TYPE_MASK;
  if (atom->sp_flag) sync_mask |= SP_MASK | FM_MASK;
  atomKK->sync(execution_space, sync_mask);
...
void MLIAPDataKokkos<DeviceType>::grow_neigharrays() {
  ...
  fm_device = atom->sp_flag ? atomKK->k_fm.view<DeviceType>().data() : nullptr;
  sp_device = atom->sp_flag ? atomKK->k_sp.view<DeviceType>().data() : nullptr;
  x_device = atomKK->k_x.view<DeviceType>().data();
